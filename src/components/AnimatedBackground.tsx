...
interface AnimatedBackgroundProps {
  variant?: 'hero' | 'about' | 'services' | 'projects' | 'workflow' | 'contact';
}
...
export default function AnimatedBackground({ variant = 'hero' }: AnimatedBackgroundProps) {
  ...
  useEffect(() => {
    ...
    particleRefs.current = Array.from({ length: particleCount }, () => ({
      x: Math.random() * 100,
      y: Math.random() * 100,
    ...
    lineRefs.current = Array.from({ length: lineCount }, () => {
    ...
    hexRefs.current = Array.from({ length: hexCount }, () => ({
  ...
  return (
    <div className="absolute inset-0 overflow-hidden z-0">
      ...
      <div className="absolute inset-0 opacity-10 font-mono text-[8px] text-[#4f46e5] select-none">
